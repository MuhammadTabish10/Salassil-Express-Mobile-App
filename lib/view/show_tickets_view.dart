...
class _ShowTicketsState extends State<ShowTicketsView> {
  ...
  void initState() {
    super.initState();
    ticketsFuture = fetchTicketsForAll();
  }
  ...
  Future<List<Ticket>> fetchTicketsForAll() async {
    try {
      String token = Provider.of<TokenProvider>(context, listen: false).token;
      List<Ticket> fetchedTickets = await getTicketsWithAllStatus(token);
      initialLoadCompleted = true;
      return fetchedTickets;
    } catch (error) {
      debugPrint('Error fetching tickets: $error');
  ...
  void _onStatusSelected(String status) {
    setState(() {
      ...
      if (status == "All") {
        ...
        ticketsFuture = fetchTickets(status);
      }
    });
  }
            ...
            children: [
              ...
              AnimatedCard(
                ...
                child: PopupMenuButton<String>(
                  ...
                  onSelected: _onStatusSelected,
                  itemBuilder: (BuildContext context) {
                    return ['All','Open', 'Closed', 'On-Hold'].map((String value) {
                      return PopupMenuItem<String>(
                        value: value,
                        child: Text(value),
                      );
